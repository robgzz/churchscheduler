...
export async function sendPushToMember({churchId,memberId,title,body,data={}}){
  ...
  const result=await getMessaging(firebaseApp()).sendEachForMulticast({tokens,notification:{title:String(title||'Westbury Church Scheduler').slice(0,120),body:String(body||'').slice(0,500)},data:Object.fromEntries(Object.entries(data||{}).map(([k,v])=>[String(k),String(v??'')]))});
  ...
  return {ok:result.successCount>0,successCount:result.successCount,failureCount:result.failureCount};
}
