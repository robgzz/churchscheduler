...
export function detectDomain(text,{actor}={}){
  ...
  const directModuleToggle=admin&&/\b(enable|enabled|disable|disabled|habilita|habilitar|activa|activar|desactiva|desactivar)\b/.test(text)&&!(/\b(miembro|member|perfil|profile|ministerio|ministry|cantos?|songs?|eligible|ineligible|puede servir|can serve|can sing)\b/.test(text));
  if(directModuleToggle){const moduleDomain=/\b(events?|eventos?)\b/.test(text)?'events':/\b(tasks?|tareas?)\b/.test(text)?'tasks':/\b(prayer|oracion|peticiones?)\b/.test(text)?'prayer':/\b(children|child care|cuidado de ninos|nursery|toddlers)\b/.test(text)?'children':/\b(publications?|publicaciones?|bulletin|boletin|announcements?|anuncios?)\b/.test(text)?'publications':/\b(worship|adoracion|programa)\b/.test(text)?'worship':'';if(moduleDomain)return {domain:moduleDomain,resource:'module',score:29,evidence:[`modules:${moduleDomain}`],second:null};}
  const memberEligibilityChange=admin&&/\b(cambia|cambiar|modifica|modificar|edita|editar|actualiza|actualizar|pon|poner|quita|quitar|agrega|agregar|incluye|incluir|activa|activar|habilita|habilitar|permite|permitir|asigna|asignar|remueve|remover|change|update|edit|add|include|enable|allow|remove|assign|eligible|ineligible|puede servir|no puede servir|no debe servir|can serve|cannot serve|can sing|no canta)\b/.test(text)&&/\b(miembro|member|perfil|profile|ministerio|ministerios|ministry|ministries|cantos?|songs?|clase|class|meditacion|sermon|peticiones|comunion|ofrenda|vigilancia|seguridad|bienvenida|lectura|oracion|prayer|worship|adoracion|miercoles|wednesday|domingo|sunday)\b/.test(text);
  ...
  const memberProfileQuery=admin&&(/\b(perfil de|ficha de|datos de|profile of|profile for|member profile)\b/.test(text)||/\b(es|esta|is|has|tiene)\b.*\b(admin|administrador|administradora|administrator|admin access|acceso de administrador)\b/.test(text));
  if(memberProfileQuery)return {domain:'members',resource:'profile',score:19,evidence:['members:profile-query'],second:null};
...
export function resolveFrame(frame,{text,actor}){const admin=Boolean(actor?.isAdmin),owner=Boolean(actor?.isOwner);let intent='unknown',boost=0;
  ...
  switch(frame.domain){
    ...
    case 'members':
      if(admin&&frame.resource==='eligibility'){const changeEvidence=(frame.evidence||[]).includes('members:eligibility-change')||/\b(excepto|menos|except)\b/.test(text);intent=changeEvidence||[Operation.UPDATE,Operation.CREATE,Operation.ASSIGN,Operation.DELETE,Operation.REQUEST,Operation.ENABLE,Operation.DISABLE].includes(frame.operation)||frame.speechAct===SpeechAct.COMMAND?'admin.memberEligibilityUpdate':'admin.memberEligibilityQuery';}else if(admin&&frame.resource==='profile')intent='admin.memberProfileQuery';else if(admin&&(frame.operation===Operation.CREATE||/\b(nuevo|nueva|new|cuenta de miembro|member account)\b/.test(text)))intent='admin.memberCreate';else if(admin)intent='admin.memberSearch';break;
