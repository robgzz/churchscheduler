...
function csvCell(v){const s=safe(v);return /[",\n]/.test(s)?`"${s.replaceAll('"','""')}"`:s;}
function dateField(r){return r.dateISO||r.occurredAt||r.createdAt||r.publishedAt||r.checkInAt||'';}
...
export async function buildReport(churchId,type,{from='',to=''}={}){
  ...
  const [members,assignments,history,notifications,content,visitors,petitions,children,checkIns,services,ministries]=await Promise.all([
    listDocs(tableNames.members,churchId,{max:10000}),listDocs(tableNames.assignments,churchId,{max:10000}),listDocs(tableNames.history,churchId,{max:10000}),listDocs(tableNames.notificationLogs,churchId,{max:10000}),listDocs(tableNames.content,churchId,{max:5000}),listDocs(tableNames.visitorContacts,churchId,{max:5000}),listDocs(tableNames.petitions,churchId,{max:5000}),listDocs(tableNames.children,churchId,{max:5000}),listDocs(tableNames.childCheckIns,churchId,{max:10000}),listDocs(tableNames.services,churchId,{max:1000}),listDocs(tableNames.ministries,churchId,{max:1000})
  ]);
  ...
  if(type==='leadership-overview'){
    summary=[['Active members',members.filter(x=>x.active!==false).length],['Worship members',members.filter(x=>(x.groups||[]).includes('worship')&&x.active!==false).length],['Assignments',af.length],['Unfilled assignments',af.filter(x=>x.status==='unfilled').length],['Replacements',hf.filter(x=>x.eventType==='replacement.requested').length],['Announcements/Bulletins',cf.length],['Visitors',vf.filter(x=>x.kind==='visitor_contact').length],['Prayer petitions',pf.length],['Children checked in',kif.length],['Notification attempts',nf.length],['Notification failures',nf.filter(x=>x.status==='failed').length]];
    ...
    rows=cf.map(c=>({Published:c.publishedAt||c.createdAt||'',Kind:c.kind||'',Title:c.titleEn||c.title||c.titleEs||'',Published:yes(c.published!==false),Address:c.address||'',Attachment:c.attachment?.fileName||''}));
...
export function reportCsv(report){const headers=Object.keys(report.rows[0]||{Message:'No records'});const meta=[['Church',report.church.churchNameEn||report.church.churchName||'Westbury Church of Christ'],['Report',report.title],['Date Range',`${report.from||'All'} to ${report.to||'All'}`],['Generated',report.generatedAt],['Logo','/api/public/church-logo'],[]];return [...meta.map(r=>r.map(csvCell).join(',')),headers.map(csvCell).join(','),...report.rows.map(r=>headers.map(h=>csvCell(r[h])).join(','))].join('\r\n');}
