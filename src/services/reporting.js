...
export async function buildReport(churchId,type,{from='',to=''}={}){
  ...
  if(type==='leadership-overview'){
    ...
    rows=events.filter(e=>inRange(e.dateISO,from,to)).map(e=>({Date:e.dateISO||'',Time:e.startTime||'',Event:e.titleEn||e.titleEs||'',Location:e.location||'',Capacity:e.capacity||'',RegisteredPeople:eventRegistrations.filter(r=>r.eventId===e.id&&r.status==='registered').reduce((n,r)=>n+Number(r.partySize||1),0),Active:yes(e.active!==false)}));
