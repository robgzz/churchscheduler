...
memberRouter.get('/push-devices',async(req,res)=>{const rows=await listMemberPushDevices(req.churchId,req.identity.member.id);res.json(rows.map(x=>({id:x.id,platform:x.platform,pushEnvironment:x.pushEnvironment||'',deviceName:x.deviceName||'',lastSeenAt:x.lastSeenAt||''})));});
memberRouter.post('/push-devices',async(req,res,next)=>{try{res.status(201).json(await upsertPushDevice(req.churchId,req.identity.member,{token:req.body?.token,platform:req.body?.platform||'android',deviceName:req.body?.deviceName||'',pushEnvironment:req.body?.pushEnvironment||''}));}catch(e){next(e);}});
