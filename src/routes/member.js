...
memberRouter.get('/children-worker',async(req,res,next)=>{try{const roles=assertChildrenWorker(req.identity.member),settings=await getDoc(tableNames.settings,req.churchId,'church')||{},timezone=settings.timezone||'America/Chicago',today=new Intl.DateTimeFormat('en-CA',{timeZone:timezone,year:'numeric',month:'2-digit',day:'2-digit'}).format(new Date());const [checkIns,members]=await Promise.all([listDocs(tableNames.childCheckIns,req.churchId,{filter:`status eq 'checked_in'`,max:1000}),listDocs(tableNames.members,req.churchId,{max:5000})]);const mm=new Map(members.map(m=>[m.id,m]));const rows=checkIns.filter(x=>String(x.dateISO||String(x.checkInAt||'').slice(0,10))===today&&roles.includes(String(x.careArea||''))).map(x=>{const parent=mm.get(x.memberId)||{};return {...x,parentName:x.parentName||parent.fullName||'',parentHasSms:Boolean(parent.phone)&&parent.childrenNotificationPreferences?.sms!==false,parentHasEmail:Boolean(parent.email)&&parent.childrenNotificationPreferences?.email!==false,parentHasPush:parent.childrenNotificationPreferences?.push!==false};}).sort((a,b)=>String(a.checkInAt||'').localeCompare(String(b.checkInAt||'')));res.json({roles,dateISO:today,children:rows});}catch(e){next(e);}});
memberRouter.post('/children-worker/check-ins/:id/alert',async(req,res,next)=>{try{const roles=assertChildrenWorker(req.identity.member),row=await getDoc(tableNames.childCheckIns,req.churchId,req.params.id);if(!row||row.status!=='checked_in'||!roles.includes(String(row.careArea||'')))return res.status(404).json({error:'Active child check-in not found for your care area.'});const parent=await getDoc(tableNames.members,req.churchId,row.memberId);if(!parent)return res.status(404).json({error:'Parent/guardian profile not found'});const prefs=parent.childrenNotificationPreferences||{},message=String(req.body.message||`${row.childName} needs you in the ${row.careArea==='nursery'?'Nursery':'Toddlers area'}. Please come to the children’s ministry check-in area.`).trim().slice(0,500),eventKey=`children-parent-alert:${row.id}:${Date.now()}`;let queued=0;for(const channel of ['sms','email','push']){if(channel==='sms'&&prefs.sms===false)continue;if(channel==='email'&&prefs.email===false)continue;if(channel==='push'&&prefs.push===false)continue;if(await enqueue({churchId:req.churchId,eventKey,channel,member:parent,subject:'Westbury Church Hub - Child care alert',message,metadata:{type:'children.parent_alert',checkInId:row.id,childId:row.childId,careArea:row.careArea,route:'children'}}))queued++;}await appendHistory(req.churchId,{eventType:'children.parent_alert',memberId:parent.id,dateISO:row.dateISO||'',source:'children-worker',details:{checkInId:row.id,childId:row.childId,childName:row.childName,careArea:row.careArea,workerId:req.identity.member.id,queued}});res.json({ok:true,queued});}catch(e){next(e);}});
