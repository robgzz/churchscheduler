...
app.use(requireCsrf);
app.get('/healthz',(req,res)=>res.json({ok:true,version:'4.5.0'}));
app.get('/readyz',async(req,res)=>{try{await ensureStorage();res.json({ok:true,version:'4.5.0'});}catch(e){res.status(503).json({ok:false,error:'storage_unavailable'});}});
...
app.listen(config.port,()=>console.log(`Westbury Church Hub V4.5.0 listening on ${config.port}`));
