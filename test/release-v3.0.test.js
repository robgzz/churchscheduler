...
test('V3 announcement activity address opens maps and is persisted',()=>{const a=read('src/routes/admin.js'),ui=read('public/assets/app.js');assert.match(a,/(?:address:String\((?:req\.body|body)\.address|contentDocFromRequest\(body)/);assert.match(ui,/maps:\/\//);assert.match(ui,/geo:0,0\?q=/);});
